    ...
    <template v-else>
      ...
      <div class="movie-detail-item" >
        ...
        <div class="movie-detail-content">
          <div class="movie-row">
            导演：
            <a class="movie-tag" :href="director.alt" v-for="(director,index) in subject.directors" target="_blank" :key='index'>{{director.name}}</a>
          </div>
          <div class="movie-row">
            主演：
            <a class="movie-tag" :href="cast.alt" v-for="(cast,index) in subject.casts" target="_blank" :key='index'>{{cast.name}}</a>
          </div>
          <div class="movie-row">
            类型：
            <el-tag class="movie-genre" type="success" v-for="(genre,index) in subject.genres" :key='index'>{{genre}}</el-tag>
          </div>
          <div class="movie-row">
            制片国家/地区：
